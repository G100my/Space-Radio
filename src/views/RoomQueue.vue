<template>
  ...
  <div class="room-queue">
    <div class="track-table">
      ...
      <Track v-for="(detailObject, key) in urgentTrack" :key="key" :info="detailObject" :is-urgent="true">
        <div class="feature-buttons">
          <button class="remove-button" type="button" @click="remove(key, 'urgent')">
            <!-- prettier-ignore -->
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16">
              <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
            </svg>
          </button>
          <button class="urgent2normal-button" type="button" @click="urgent2normal(key)">
            <!-- prettier-ignore -->
          ...
          <button class="edit-button" type="button" @click="editNote(key, detailObject.name)">
            <!-- prettier-ignore -->
      ...
      <Track v-for="(detailObject, key) in normalTrack" :key="key" :info="detailObject">
        <div class="feature-buttons">
          <button class="remove-button" type="button" @click="remove(key, 'normal')">
            <!-- prettier-ignore -->
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16">
              <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
            </svg>
          </button>
          <button class="normal2urgent-button" type="button" @click="normal2urgent(key, detailObject.name)">
            <!-- prettier-ignore -->
...
export default {
  ...
  methods: {
    ...
    editNote(key, trackName) {
      const submitFunction = newNote => {
        this.$store.dispatch(`urgentEdit`, { key, note: newNote })
      }
      this.$emit('activeNoteDialog', { queueKey: key, level: 'urgent', trackName, submitFunction })
    },
    urgent2normal(key) {
      this.$store.dispatch('urgent2normal', key)
    },
    normal2urgent(key, trackName) {
      const submitFunction = newNote => {
