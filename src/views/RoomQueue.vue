<template>
  ...
  <div class="room-queue">
    <div class="track-table">
      ...
      <Track v-for="(detailObject, queueKey) in urgentTrack" :key="queueKey" :info="detailObject" :is-urgent="true">
        <div class="feature-buttons">
          ...
          <button class="edit-button" type="button" @click="editNote(queueKey, detailObject.name)">
            <!-- prettier-ignore -->
      ...
      <Track v-for="(detailObject, queueKey) in normalTrack" :key="queueKey" :info="detailObject">
        <div class="feature-buttons">
          ...
          <button class="normal2urgent-button" type="button" @click="normal2urgent(queueKey, detailObject.name)">
            <!-- prettier-ignore -->
...
export default {
  ...
  methods: {
    ...
    editNote(queueKey, trackName) {
      const submitFunction = newNote => {
        this.$store.dispatch(`urgentEdit`, { queueKey, note: newNote })
      }
      this.$emit('activeNoteDialog', { queueKey, level: 'urgent', trackName, submitFunction })
    },
    urgent2normal(queueKey) {
      this.$store.dispatch('urgent2normal', queueKey)
    },
    normal2urgent(queueKey, trackName) {
      const submitFunction = newNote => {
        this.$store.dispatch('normal2urgent', { queueKey, note: newNote })
      }
      this.$emit('activeNoteDialog', { queueKey, level: 'normal', trackName, submitFunction })
    },
  },
}
</script>
